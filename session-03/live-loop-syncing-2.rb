...
# filename: live-loop-syncing-2.rb
...
live_loop :beat do
  16.times do
    sample :elec_blip, amp: 0.5, rate: 3
    sleep 1
  end
end
...
live_loop :two_bars, sync: :beat do
  sample :elec_blip, amp: 0.25, rate: 1
  sleep 8
end

# Synch 4-bar patterns
live_loop :four_bars, sync: :two_bars do
  sample :elec_blip, amp: 0.5, rate: 0.25
  sleep 16
end
...
# The Chords
live_loop :chords, sync: :four_bars do
  stop if chords < 1
...
live_loop :bass, sync: :four_bars do
  ...
  with_synth :fm do
    use_synth_defaults amp: 1.5, attack: 0.0, sustain: 0.15, release: 0.01, cutoff: 65
...
# Bass Drum and Snare
live_loop :bass_snare, sync: :two_bars do
  stop if drums < 1
...
live_loop :hihat1, sync: :two_bars do
  stop if hihat1 < 1

  (4*2).times do
    sleep 0.25
    sample :drum_cymbal_closed, amp: 0.5 * hihat1_vol, finish: 1
    sleep 0.25
    sample :drum_cymbal_closed, amp: 0.35 * hihat1_vol, finish: 0.1
    sleep 0.25
    sample :drum_cymbal_closed, amp: 0.15 * hihat1_vol, finish: 0.2
    sleep 0.25
    sample :drum_cymbal_closed, amp: 0.35 * hihat1_vol, finish: 0.1
    #sleep 0.25
  end
...
# Hihat2
hihat2_vol = 0.5
live_loop :hihat2, sync: :two_bars do
  stop if hihat2 < 1
  (4*2).times do
    sample :drum_cymbal_closed, amp: 0.5 * hihat2_vol
    sleep 0.25
    sample :drum_cymbal_closed, amp: 0.75 * hihat2_vol
    sleep 0.25
    sample :drum_cymbal_closed, amp: 0.5 * hihat2_vol
    sleep 0.25
    sample :drum_cymbal_closed, amp: 0.5 * hihat2_vol
...
live_loop :brass1, sync: :four_bars do
  stop if brass1 < 1
...
live_loop :brass2, sync: :four_bars do
  stop if brass2 < 1
  ...
  use_synth :fm
  use_synth_defaults amp: brass2_lv, attack: 0, sustain: 0, release: 0.15, depth: 2, divisor: 0.5
...
# The Flitter
live_loop :flitter, sync: :four_bars do
  stop if flittr < 1
